...
class UserSkills(commands.Cog):
    ...
    async def upgrade(self, ctx, upgrade: typing.Optional[str] = '', amount: typing.Optional[str] = '1'):
        ...
        if upgrade == 'help' or upgrade == '':
            ...
            embed.add_field(name=f'🌀 Critical Power (Level {round(cp_level)})', value=f'Factor the points are multiplied by \n Factor: {2+(cp_level)}x \n Upgrade Cost: {round(cp_cost):,} {silver_emoji}', inline=False)
